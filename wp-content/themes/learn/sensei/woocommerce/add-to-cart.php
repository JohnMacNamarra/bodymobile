...
if ( WooThemes_Sensei_Utils::sensei_customer_bought_product( $current_user->user_email, $current_user->ID, $wc_post_id ) ) { ?>
    <div class="sensei-message tick"><?php _e( 'You are currently taking this course.', 'learn' ); ?></div>
<?php } else {
                ...
                <?php if (! sensei_check_if_product_is_in_cart( $wc_post_id ) ) { ?>
                    <form action="<?php echo esc_url( $product->add_to_cart_url() ); ?>" class="cart" method="post" enctype="multipart/form-data">
                        ...
                        <?php if ( isset( $product->variation_id ) && 0 < intval( $product->variation_id ) ) { ?>
                            ...
                            <?php } ?>
                        <?php } ?>
                        <button type="submit" class="single_add_to_cart_button button_fullwidth-3"><?php echo apply_filters('single_add_to_cart_text', __('Take this course', 'learn'), $product->product_type); ?> <?php //echo $product->get_price_html(); ?></button>
                    </form>
